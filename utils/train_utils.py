...
def init_random_seed(seed=None, device='cuda'):
    ...
    rank, world_size = get_dist_info()      # 不设定时，返回rank=0，world_size=1
...
def file2dict(filename):    #
    (path, file) = os.path.split(filename)  # 分离文件名和相对路径

    abspath = os.path.abspath(os.path.expanduser(path))     # 获取绝对路径
    sys.path.insert(0,abspath)      # 定义搜索路径的优先顺序，序号从0开始，表示最大优先级
    mod = importlib.import_module(file.split('.')[0])       # 动态地获取另一个py文件中定义好的变量/方法
    sys.path.pop(0)
    cfg_dict = {
                name: value
                ...
                and not isinstance(value, types.FunctionType)
                    }   # 对mod中数据进行解析和筛选，以字典方式进行返回，当键值不存在时，返回值为None，不会引发异常。
...
def print_info(cfg):
    ...
    table_instance = AsciiTable(TABLE_DATA,TITLE)   # AsciiTable(data,title)，以表格形式打印数据
    print()
